...
requirements
- the latest version [hare programming language](https://hare-lang.org). build from source
- [QBE](https://c9x.me/compile/) backend (requirement for the hare lang as well)
- a posix compatible gnu assembler and linker (requirement for the hare land and qbe as well)

once everything is set, simply run the `install.sh`. it will copy a binary called `lcc` and the runtime and standard library to `~/.local/bin` and `~/.local/lib/lcc`, respectively, and will be available only for your current user. you also can specify which directory the compiler should look for the runtime and standard library using the `-std` flag.
